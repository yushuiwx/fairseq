...
@dataclass
class PolynomialDecayStageLRScheduleConfig(FairseqDataclass):
    ...
    stage_interval: int = field(
        default=0,
        metadata={"help": ""},
    )
    shrink_ratio: float = field(
        default=0,
        metadata={"help": ""},
    )
...
class PolynomialDecayStageLRSchedule(FairseqLRScheduler):
    """Decay the LR on a fixed schedule."""

    def __init__(self, cfg: PolynomialDecayStageLRScheduleConfig, optimizer):
    ...
    def step_update(self, num_updates):
        ...
        if self.cfg.warmup_updates > 0 and num_updates <= self.cfg.warmup_updates:
            ...
            stage = num_updates // self.stage_interval
            warmup = self.cfg.warmup_updates
            lr_range = self.lr * (1 - stage * self.shrink_ratio) - self.end_learning_rate
