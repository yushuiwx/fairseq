...
class MoECriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        3) logging outputs to display while training
        """
        loss, inner_loss, moe_loss, KL_loss, moe_metadata, sample_size, logging_output = self.compute_loss(model, sample, reduce=reduce)
    ...
    def compute_loss(self, model, sample, reduce=True):
        ...
        loss = inner_loss + self.gate_loss_weight * gate_loss + 1000.0 * KL_loss
    ...
    def reduce_moe_metrics(logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "KL_loss", KL_loss_sum / sample_size, sample_size, round=8
        )
